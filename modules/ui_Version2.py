...
import cv2
from cv2_enumerate_cameras import enumerate_cameras  # Añadido import
...
# Constantes de la UI (tamaños, etc.)
ROOT = None
POPUP = None
POPUP_LIVE = None
ROOT_HEIGHT = 800
...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    # --- Fin selección de cámara ---

    # 1) Definir DoubleVar para transparencia (0 = totalmente transparente, 1 = totalmente opaco)
    ...
    def on_transparency_change(value: float):
        ...
        val = float(value)
        modules.globals.opacity = val  # Ajustar opacidad global
        percentage = int(val * 100)

        if percentage == 0:
            modules.globals.fp_ui["face_enhancer"] = False
            update_status("Transparencia 0% - Intercambio de caras deshabilitado.")
        elif percentage == 100:
            modules.globals.face_swapper_enabled = True
            update_status("Transparencia 100%.")
        else:
            modules.globals.face_swapper_enabled = True
            update_status(f"Transparencia configurada a {percentage}%")

    # 2) Etiqueta y slider de transparencia (colocados ARRIBA de nitidez)
    transparency_label = ctk.CTkLabel(root, text="Transparencia:")
    ...
    # 3) Etiqueta y slider de nitidez
    sharpness_var = ctk.DoubleVar(value=modules.globals.sharpness)  # valor inicial desde globals
    def on_sharpness_change(value: float):
        modules.globals.sharpness = float(value)
        update_status(f"Nitidez configurada a {value:.1f}")

    sharpness_label = ctk.CTkLabel(root, text="Nitidez:")
    ...
    # Estado y enlace en la parte inferior
    global status_label
...
def analyze_target(start: Callable[[], None], root: ctk.CTk):
    if POPUP != None and POPUP.winfo_exists():
        update_status("Por favor complete el pop-up o ciérrelo.")
        return
    ...
    if modules.globals.map_faces:
        ...
        if is_image(modules.globals.target_path):
            update_status("Obteniendo caras únicas")
            get_unique_faces_from_target_image()
        ...
        if len(modules.globals.source_target_map) > 0:
            ...
            update_status("No se encontraron caras en el objetivo")
    else:
...
def create_source_target_popup(
    ...
    def on_submit_click(start):
        if has_valid_map():
            ...
            update_pop_status("¡Se requiere al menos 1 origen con objetivo!")
...
def update_popup_source(
    ...
    if source_path == "":
        ...
        if face:
            ...
            update_pop_status("¡No se pudo detectar una cara en la última carga!")
        return map
...
def update_status(text: str) -> None:
    # Actualiza la etiqueta de estado (la función _ buscará la traducción si existe)
...
def update_tumbler(var: str, value: bool) -> None:
    # Actualiza el conmutador en globals y guarda estado
    modules.globals.fp_ui[var] = value
    save_switch_states()
    # Si estamos en previsualización en vivo, actualizar procesadores de frame
...
def check_and_ignore_nsfw(target, destroy: Callable = None) -> bool:
    """Comprobar si el objetivo es NSFW.
    TODO: Considerar aplicar desenfoque al objetivo.
    """
    from numpy import ndarray
    from modules.predicter import predict_image, predict_video, predict_frame

    if type(target) is str:  # ruta de archivo (imagen/video)
        check_nsfw = predict_image if has_image_extension(target) else predict_video
    elif type(target) is ndarray:  # frame
        check_nsfw = predict_frame
    if check_nsfw and check_nsfw(target):
        if destroy:
            destroy(
                to_quit=False
            )  # No es necesario destruir la ventana si el objetivo es NSFW
        update_status("¡Procesamiento ignorado!")
        return True
    else:
        return False
...
def fit_image_to_size(image, width: int, height: int):
    # Ajusta la imagen manteniendo proporciones para adaptarla a un tamaño dado
...
def update_preview(frame_number: int = 0) -> None:
    if modules.globals.source_path and modules.globals.target_path:
        ...
        update_status("¡Procesamiento completado!")
        PREVIEW.deiconify()
...
def webcam_preview(root: ctk.CTk, camera_index: int):
    ...
    if POPUP_LIVE and POPUP_LIVE.winfo_exists():
        update_status("El mapeador Fuente x Objetivo ya está abierto.")
        POPUP_LIVE.focus()
        return

    if not modules.globals.map_faces:
        if modules.globals.source_path is None:
            update_status("Por favor seleccione primero una imagen de origen")
            return
...
def get_available_cameras():
    """Devuelve una lista de nombres e índices de cámaras disponibles."""
    if platform.system() == "Windows":
        try:
            graph = FilterGraph()
            devices = graph.get_input_devices()

            # Crear lista de índices y nombres
            camera_indices = list(range(len(devices)))
            camera_names = devices

            # Si no hay cámaras detectadas por DirectShow, intentar fallback con OpenCV
            if not camera_names:
                # Probar abrir la cámara con índice -1 y 0
                test_indices = [-1, 0]
                working_cameras = []
            ...
            # Si aún no hay cámaras, devolver listas vacías
            if not camera_names:
        ...
        # Detección de cámaras en sistemas tipo Unix (Linux/Mac)
        camera_indices = []
        camera_names = []

        if platform.system() == "Darwin":  # Manejo específico macOS
            # Intentar abrir la FaceTime camera por defecto primero
            ...
            # En macOS, cámaras adicionales suelen usar índices 1 y 2
            for i in [1, 2]:
            ...
            # Detección en Linux: probar los primeros 10 índices
            for i in range(10):
                cap = cv2.VideoCapture(i)
                if cap.isOpened():
                    camera_indices.append(i)
...
def create_webcam_preview(camera_index: int):
    ...
    if not cap.start(PREVIEW_DEFAULT_WIDTH, PREVIEW_DEFAULT_HEIGHT, 60):
        update_status("Error al iniciar la cámara")
        return
...
def create_source_target_popup_for_webcam(
    ...
    def on_submit_click():
        if has_valid_map():
            simplify_maps()
            update_pop_live_status("¡Mapeos enviados con éxito!")
            create_webcam_preview(camera_index)  # Abrir ventana de previsualización
        else:
            update_pop_live_status("¡Se requiere al menos 1 origen con objetivo!")

    def on_add_click():
        add_blank_map()
        refresh_data(map)
        update_pop_live_status("¡Por favor proporciona mapeos!")

    def on_clear_click():
        clear_source_target_images(map)
        refresh_data(map)
        update_pop_live_status("¡Todos los mapeos fueron borrados!")
...
def update_webcam_source(
    ...
    if source_path == "":
        ...
        if face:
            ...
            update_pop_live_status("¡No se pudo detectar una cara en la última carga!")
        return map
